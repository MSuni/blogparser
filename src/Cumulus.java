...
public class Cumulus {
  ...
  File file = new File("/users/Blackstorm/cumulus_reviews_threestar.txt");
  ...
  public void usehtmlunit() {
	...
	for (int pageFor = 0; pageFor < pageLinks.size(); pageFor++) {
	  ...
	  if (pageFor != 0) {
	    page = webClient.getPage("https://www.tripadvisor.fi" + pageLinks.get(pageFor).getValue().toString());
	    System.out.println("going to page " + pageFor);
	    bw.write("going to page " + pageFor + " " + pageLinks.get(pageFor).getValue().toString());
		  bw.flush();
	  ...
	  for (int reviewFor = 0; reviewFor < reviewLinks.size(); reviewFor++) {
	    ...
	    List<DomAttr> rating = null;
	    List<DomAttr> rating2 = null;
	    //reviewRating
	    try {
	      bw.write("checking review: " + reviewFor + " ");
		  bw.flush();
	      System.out.println("try");
	      rating = (List) page.getByXPath("//img[@class='sprite-rating_s_fill rating_s_fill s30']/@property");
	      rating2 = (List) page.getByXPath("//img[@class='sprite-rating_no_fill rating_no_fill no30']/@alt");
	      if (rating.get(0).toString() != "" || rating2.get(0).getValue().toString() == "3/5 tähteä") {
		...
		if (!textDiv.isEmpty()) {
		  ...
		  String text = hotelFor + " " + pageFor + " " + textDiv.get(0).getTextContent() + System.getProperty("line.separator");
		  text.replaceAll("(\\r|\\n)", " ");
